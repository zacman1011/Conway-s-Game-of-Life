...
class PaddedQuad(Board):

    def __init__(self, seed_coords, h=50, w=50, rules=None, padding=0):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if not (i == 0 and j == 0):
                    ...
                    if 0 <= x_neighbour < self.w and 0 <= y_neighbour < self.h:
                        num_neighbours += self.board[y_neighbour][x_neighbour]
                    else:
                        num_neighbours += self.padding
